...
project_name = config.get("project_name") or "credicorp-de"
enviroment = config.get("enviroment") or "dev"
location = config.get("location") or "eastus2"

def get_resource_name(resource_type: str) -> str:
    return f"{resource_type}-{project_name}-{enviroment}"
...
storage_account = create_data_lake(
    name=f"st{project_name.replace('-','')}{enviroment}",
...
pulumi.export("key_vault_uri", key_vault.vault_uri)
